...
@section('content')
    ...
    <main class="relative min-h-screen flex items-center justify-center py-12 px-5 overflow-hidden">
        ...
        <section class="relative w-full max-w-lg">
            <div class="flex flex-col w-full rounded-[20px] border border-rebel-red-900/30 p-8 gap-6 bg-rebel-black-1000/80 backdrop-blur-sm shadow-2xl">
                <div class="text-center">
                    <h1 class="font-bold text-[22px] leading-[33px] mb-2 text-beige-50">Reset Password</h1>
                    <p class="text-sm text-beige-300">Pilih metode untuk menerima link reset password</p>
                </div>

                <!-- Email Option -->
                <div class="space-y-4">
                    <div class="border border-rebel-red-900/40 bg-rebel-black-900/30 rounded-xl p-4 hover:border-gold-500 hover:bg-rebel-black-900/50 transition-all duration-300 cursor-pointer" 
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="flex-1">
                                <h3 class="font-semibold text-beige-50">Reset via Email</h3>
                                <p class="text-sm text-beige-300">Kirim link reset password ke email Anda</p>
                            </div>
                            <div class="w-6 h-6 border-2 border-rebel-red-900/40 rounded-full flex items-center justify-center">
                                <div class="w-3 h-3 bg-gold-500 rounded-full hidden" id="email-check"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border border-rebel-red-900/40 bg-rebel-black-900/30 rounded-xl p-4 hover:border-gold-500 hover:bg-rebel-black-900/50 transition-all duration-300 cursor-pointer" 
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="flex-1">
                                <h3 class="font-semibold text-beige-50">Reset via WhatsApp</h3>
                                <p class="text-sm text-beige-300">Kirim link reset password ke WhatsApp Anda</p>
                            </div>
                            <div class="w-6 h-6 border-2 border-rebel-red-900/40 rounded-full flex items-center justify-center">
                                <div class="w-3 h-3 bg-gold-500 rounded-full hidden" id="whatsapp-check"></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <form id="email-form" method="POST" action="{{ route('password.email') }}" class="hidden space-y-4">
                    ...
                    <button type="submit"
                        class="w-full flex items-center justify-center gap-[10px] rounded-full py-[14px] px-5 bg-gold-600 hover:bg-gold-500 shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer">
                        <span class="font-semibold text-charcoal-900">Kirim Link Reset via Email</span>
                    </button>
                </form>
                ...
                <form id="whatsapp-form" method="POST" action="{{ route('password.whatsapp') }}" class="hidden space-y-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                        <p class="text-xs text-beige-400">Link reset akan dikirim ke nomor WhatsApp yang terdaftar pada akun email ini</p>
                    </div>
                    <button type="submit"
                        class="w-full flex items-center justify-center gap-[10px] rounded-full py-[14px] px-5 bg-green-600 hover:bg-green-500 shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer">
                ...
                <!-- Back to Login -->
                <div class="text-center">
                    <a href="{{ route('login') }}" class="text-sm text-beige-300 hover:text-gold-400 transition-colors cursor-pointer">
                        ← Kembali ke Login
                    </a>
                </div>
            </div>
        ...
        function selectResetMethod(method) {
            ...
            document.querySelectorAll('.border').forEach(el => {
                if (el.classList.contains('rounded-xl')) {
                    el.classList.remove('border-gold-500');
                    el.classList.add('border-rebel-red-900/40');
                }
            });
            ...
            if (method === 'email') {
                ...
                event.currentTarget.classList.remove('border-rebel-red-900/40');
